...
int main() {
    ...
    vector<unordered_map<int, unordered_set<int>>> transitions1(n + 2);
    for (int i = 0; i < m; i++) {
        fin >> _1 >> _2 >> _3;
        transitions1[_1][_3].emplace(_2);
    }

    queue<unordered_set<int>> P;
    P.push({1});
    unordered_set<int> qd;
    unordered_set <unordered_set<int>> Qd;
    unordered_set<int> nowCond;
    while (!P.empty()) {
        nowCond = P.front();
        P.pop();
        for (auto el : transitions1[nowCond]) {
            qd.clear();
            for
            qd = el.second;
        }
